...
Для OpenFeign настроены ограниченные `connectTimeout` и `readTimeout`, чтобы сервисы быстро реагировали на недоступность зависимостей и не зависали на сетевых вызовах.

Для необязательных агрегированных данных применяется graceful degradation: если `comment-service` временно недоступен, `event-service` продолжает отдавать событие со значением `comments = 0`.
